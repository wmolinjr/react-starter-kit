...
class AddonCatalogController extends Controller implements HasMiddleware
{
    ...
    protected function transformAddon(Addon $addon): array
    {
        return [
            ...
            'provider_product_ids' => $addon->provider_product_ids,
            'provider_price_ids' => $addon->provider_price_ids,
            'provider_meter_ids' => $addon->provider_meter_ids,
            'features' => $addon->features ?? [],
            'icon' => $addon->icon ?? 'Package',
            'icon_color' => $addon->icon_color ?? 'slate',
            'badge' => $addon->badge,
            'is_synced' => ! empty($addon->provider_product_ids),
